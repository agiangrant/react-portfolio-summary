...
class _ServerMock {
  constructor() {
    ...
    getSecurityPrices = (ids) => {
      if (!Array.isArray(ids)) { return this._buildAPIErrorResponse('Ids param must be an array'); }

      const data = {};
      ids.forEach((id) => {
        if (!this._securitiesLookup[id]) { return null; }
    ...
    _buildAPIResponse = (data) => new Promise((res, rej) => setTimeout(() => res({ status: 200, data }), this._latencyBase))

    _buildAPIErrorResponse = (errorMessage) => new Promise((res, rej) => setTimeout(() => res({ status: 400, error: errorMessage }), 1))

    randomizePrices = () => {
      [this._costBasisLookup, this._openPriceLookup, this._prevClosePriceLookup].forEach(priceSet => {
        Object.keys(priceSet).forEach((k) => {
          priceSet[k] += (Math.random() * 10) - 5;
        });
      });
    }
}
